...
# Saída FIXA (o que seu front espera por posição):
# Servidor, Jogadores Online, Versão, Origem, Observação
...
COLS_IN  = ["Servidor","Versão","Jogadores Online","Origem","Observação"]
COLS_OUT = ["Servidor","Jogadores Online","Versão","Origem","Observação"]  # <- ORDEM FIXA
...
def ler_entrada():
    if not IN_ARQ.exists():
        print("❌ 'resultado_validado.csv' não encontrado."); raise SystemExit(1)
    rows = []
    with IN_ARQ.open("r", encoding="utf-8", newline="") as f:
        r = csv.DictReader(f)
        for c in COLS_IN:
            if c not in r.fieldnames:
                print(f"❌ Coluna ausente: {c}"); raise SystemExit(1)
        for row in r:
            serv = (row.get("Servidor") or "").strip()
            if not serv or serv == "===": continue
            versao = (row.get("Versão") or "").strip() or "-"   # nunca vazio pra não “andar”
            online = to_int(row.get("Jogadores Online", 0))
            origem = (row.get("Origem") or "").strip() or "Pendência"
            observ = (row.get("Observação") or "").strip()
            rows.append({"Servidor": serv, "Versão": versao, "Jogadores Online": online,
                         "Origem": origem, "Observação": observ})
    return rows

def salvar(path: Path, rows):
...
def main():
    ...
    salvar(OUT_ARQ, rows)
    salvar(OUT_ARQ_PUBLIC, rows)
    preview(OUT_ARQ)
    preview(OUT_ARQ_PUBLIC)
    print("\n✅ ranking_final.csv gerado na ordem: Servidor, Jogadores Online, Versão, Origem, Observação")
