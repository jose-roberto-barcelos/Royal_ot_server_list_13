...
# Colunas FIXAS e na ordem certa: Servidor,Versão,Jogadores Online,Origem,Observação

import asyncio, socket, struct, sys, re, csv
...
def parse_status_response(full: bytes) -> Dict:
    ...
    while True:
        ...
        try:
            if code == 0x20: out["players_info"] = (b.u32(), b.u32(), b.u32())
            elif code == 0x21:
                count = b.u32(); lst=[]
                for _ in range(min(count, 5000)): lst.append((b.str16(), b.u32()))
    ...
    try:
        ...
        def _do():
            with socket.create_connection((host, port), timeout=TIMEOUT_S) as s:
                s.settimeout(TIMEOUT_S); s.sendall(make_packet(flag))
                hdr = recv_exact(s, 2);  if not hdr: return None
                (plen,) = struct.unpack("<H", hdr)
                if plen <= 0 or plen > 65535: return None
                payload = recv_exact(s, plen);  if not payload: return None
                return hdr + payload
...
async def consulta_status(host: str, porta: int):
    ...
    if data:
        p = parse_status_response(data); players_info = p.get("players_info")
        if players_info:
            await asyncio.sleep(ESPERA_STATUS_S)
            data_sw = await fetch_once(host, porta, 0x80)
            if data_sw: software_info = (parse_status_response(data_sw).get("software_info") or {})
...
def suspeita_txt(online: int, nomes: List[str]) -> str:
    if online < 100: return ""
    from collections import Counter
    if not nomes: return "SUSPEITA: Sem lista estendida mesmo com online alto"
    c = Counter(nomes)
    dups = sum(1 for _, q in c.items() if q > 1)
    dup_ratio = dups / max(1, len(nomes))
    if dup_ratio >= 0.20: return f"SUSPEITA: muitos duplicados (~{dup_ratio:.0%})"
    return ""
...
async def processar_host(hostline: str, sem: asyncio.Semaphore) -> Dict:
    ...
    if software_info:
        for k in ("version", "version_str"):
            if software_info.get(k): versao = software_info[k]; break
    ...
    if not players_info:
        for palt in PORTAS_PADRAO:
            ...
            if players_info:
                porta = palt
                if not versao and software_info2:
                    for k in ("version", "version_str"):
                        if software_info2.get(k): versao = software_info2[k]; break
                break

    if not players_info:
        print(f"OK: {host}:{porta} => PENDENTE")
        return {"Servidor": f"{host.lower()}:{porta}", "Versão": versao or "",
                "Jogadores Online": 0, "Origem": "Pendência",
    ...
    if online >= 100:
        ...
        nomes = [n for n,_ in lst]
        obs = suspeita_txt(online, nomes)

    print(f"OK: {host}:{porta} => {online} ({versao}) {obs}")
    return {"Servidor": f"{host.lower()}:{porta}", "Versão": versao or "",
            "Jogadores Online": int(online), "Origem": "Socket",
            "Observação": obs}

async def main():
    entrada = Path(ARQ_ENTRADA)
    if not entrada.exists():
        print(f"❌ Arquivo não encontrado: {ARQ_ENTRADA}"); sys.exit(1)
    ...
    with open(CSV_SAIDA, "w", newline="", encoding="utf-8") as f:
        ...
        for r in resultados:
            if not isinstance(r["Jogadores Online"], int):
                try: r["Jogadores Online"] = int(r["Jogadores Online"])
                except: r["Jogadores Online"] = 0
            w.writerow(r)

    print(f"\n✅ Gerado {CSV_SAIDA} ({len(resultados)} linhas).")
